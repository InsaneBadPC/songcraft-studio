...
/* eslint-disable react-hooks/set-state-in-effect -- formulář se záměrně inicializuje po načtení skladby */
...
type SongForm = { title: string; albumId: string | null; stylePrompt: string; lyrics: string; notes: string; coverStorageKey: string | null; coverUrl: string | null };
...
export function SongEditor({ songId }: { songId?: string }) {
  ...
  const generateCover = async () => {
    if (!songId) { Alert.alert("Nejprve skladbu ulož", "AI cover se vytváří z uloženého názvu, promptu a textu. Nejdřív skladbu ulož, potom ji můžeš nechat ilustrovat."); return; }
    if (!form.stylePrompt.trim() && !form.lyrics.trim()) { Alert.alert("Doplň obsah skladby", "Pro lepší cover zapiš alespoň prompt pro styl nebo část textu písně."); return; }
    setCoverGenerating(true);
    try {
      const created = await createCoverGeneration.mutateAsync({ entityType: "song", entityId: songId });
      for (let attempt = 0; attempt < 24; attempt += 1) {
        await new Promise((resolve) => setTimeout(resolve, 7500));
        const status = await checkCoverGeneration.mutateAsync({ entityType: "song", entityId: songId, jobId: created.jobId });
