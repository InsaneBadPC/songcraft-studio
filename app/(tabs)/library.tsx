...
import { CoverArt, EmptyState, LoadingState, SectionTitle, StudioHeader } from "@/components/studio-ui";
...
export default function LibraryScreen() {
  ...
  const songs = useMemo(() => (snapshot.data?.songs ?? []).filter((song) => (!albumId || song.albumId === albumId) && song.title.toLowerCase().includes(search.toLowerCase())), [albumId, search, snapshot.data?.songs]);
  ...
  return <ScreenContainer className="px-5"><FlatList data={songs} keyExtractor={(item) => String(item.id)} showsVerticalScrollIndicator={false} contentContainerStyle={styles.content} ListHeaderComponent={<>
    <StudioHeader eyebrow="Dokončené nápady" title="Knihovna skladeb" />
  ...
  </>} renderItem={({ item }) => { const source = snapshot.data?.documents.find((document) => document.id === item.sourceDocumentId); const versions = snapshot.data?.versions.filter((version) => version.songId === item.id).length ?? 0; const album = snapshot.data?.albums.find((entry) => entry.id === item.albumId); return <Pressable onPress={() => router.push(`/song/${item.id}` as never)} style={({ pressed }) => [styles.row, { backgroundColor: colors.surface, borderColor: colors.border, opacity: pressed ? 0.7 : 1 }]}><CoverArt uri={source?.coverUrl ?? album?.coverUrl} title={item.title} size={62} /><View style={styles.copy}><Text numberOfLines={1} style={[styles.songName, { color: colors.foreground }]}>{item.title}</Text><Text numberOfLines={1} style={[styles.songMeta, { color: colors.muted }]}>{album?.name ?? "Bez alba"} · {versions} {versions === 1 ? "verze" : versions < 5 ? "verze" : "verzí"}</Text><View style={styles.ready}><MaterialIcons name="check-circle" size={13} color={colors.success} /><Text style={[styles.readyText, { color: colors.success }]}>text a prompt připraveny</Text></View></View><MaterialIcons name="chevron-right" size={23} color={colors.muted} /></Pressable>; }} ListEmptyComponent={<EmptyState icon="library-music" title="Knihovna je zatím prázdná" text="Označ text jako hotový. Vznikne zde skladba s promptem, textem a prostorem pro MP3 verze." action={<Pressable onPress={() => router.push("/(tabs)/texts" as never)} style={[styles.login, { backgroundColor: colors.primary }]}><Text style={styles.loginText}>Otevřít texty</Text></Pressable>} />} />
  </ScreenContainer>;
}
