...
export default function SongDetailScreen() {
  ...
  const prepareManagedCopy = trpc.studio.prepareManagedCopy.useMutation();
  const [taggingVersionIds, setTaggingVersionIds] = useState<string[]>([]);
  ...
  const prepareCopyInBackground = (versionId: string, label: string, note: string | null) => {
    setTaggingVersionIds((current) => [...current, versionId]);
    void prepareManagedCopy.mutateAsync({ id: versionId, label, note })
      .then(() => utils.studio.snapshot.invalidate())
      .catch(() => Alert.alert("MP3 je uložená", "Bezpečná cloudová kopie s ID3 tagy se zatím nepřipravila. Skladbu i původní MP3 můžeš dál otevřít a poslouchat."))
      .finally(() => setTaggingVersionIds((current) => current.filter((id) => id !== versionId)));
  };
  const saveMp3 = async (label: string, note: string) => {
    ...
    try {
      ...
      setPendingMp3(null);
      prepareCopyInBackground(versionId, label, note || null);
    } catch (error) {
  ...
  const importMultiple = async () => {
    ...
    try {
      for (const [index, asset] of validAssets.entries()) {
        ...
        prepareCopyInBackground(versionId, label, "Hromadný import");
      }
      await utils.studio.snapshot.invalidate();
