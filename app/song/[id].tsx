...
export default function SongDetailScreen() {
  ...
  const versions = snapshot.data?.versions.filter((entry) => entry.songId === songId) ?? [];
  ...
  return <ScreenContainer edges={["top", "bottom", "left", "right"]}><ScrollView contentContainerStyle={styles.content}>
    ...
    <VersionAudioPlayer versions={versions} />
    ...
    {versions.length ? versions.map((version) => <Pressable key={version.id} onPress={() => setSelectedVersion(version.id)} style={({ pressed }) => [styles.versionRow, { backgroundColor: colors.surface, borderColor: colors.border, opacity: pressed ? 0.7 : 1 }]}><View style={[styles.versionIcon, { backgroundColor: `${colors.primary}20` }]}><MaterialIcons name={version.isPrimary ? "star" : "music-note"} size={21} color={colors.primary} /></View><View style={styles.versionCopy}><View style={styles.versionTitleRow}><Text numberOfLines={1} style={[styles.versionLabel, { color: colors.foreground }]}>{version.label}</Text>{version.isPrimary ? <Text style={[styles.mainBadge, { color: colors.success }]}>HLAVNÍ</Text> : null}</View><Text numberOfLines={1} style={[styles.versionFile, { color: colors.muted }]}>{version.originalFileName} · {formatFileSize(version.byteSize)}</Text></View><MaterialIcons name="edit" size={19} color={colors.muted} /></Pressable>) : <EmptyState icon="audio-file" title="Zatím žádná zvuková verze" text="Přidej MP3 z telefonu nebo počítače. Ke každé skladbě můžeš vést více verzí." action={<PrimaryButton label="Přidat první MP3" icon="upload-file" onPress={() => void addMp3()} />} />}
...
function VersionEditor({ id, onClose }: { id: number | null; onClose: () => void }) {
  ...
  const [form, setForm] = useState({ label: "", title: "", artist: "", album: "", track: "", year: "", genre: "", comment: "" });

  useEffect(() => {
    if (version) setForm({ label: version.label, title: version.id3Title ?? "", artist: version.id3Artist ?? "", album: version.id3Album ?? "", track: version.id3TrackNumber ?? "", year: version.id3Year ?? "", genre: version.id3Genre ?? "", comment: version.id3Comment ?? "" });
  }, [version]);
  if (!version || id === null) return null;

  const payload = (isPrimary = version.isPrimary) => ({ id: version.id, label: form.label.trim() || version.label, id3Title: form.title || null, id3Artist: form.artist || null, id3Album: form.album || null, id3TrackNumber: form.track || null, id3Year: form.year || null, id3Genre: form.genre || null, id3Comment: form.comment || null, isPrimary });
  ...
  const makePrimary = async () => { try { const related = snapshot.data?.versions.filter((entry) => entry.songId === version.songId) ?? []; await Promise.all(related.map((entry) => update.mutateAsync(entry.id === version.id ? payload(true) : { id: entry.id, label: entry.label, id3Title: entry.id3Title, id3Artist: entry.id3Artist, id3Album: entry.id3Album, id3TrackNumber: entry.id3TrackNumber, id3Year: entry.id3Year, id3Genre: entry.id3Genre, id3Comment: entry.id3Comment, isPrimary: false }))); await utils.studio.snapshot.invalidate(); } catch { Alert.alert("Změna hlavní verze se nezdařila", "Zkus to znovu."); } };
