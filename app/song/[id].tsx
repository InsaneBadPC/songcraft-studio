  ...
  return <ScreenContainer edges={["top", "bottom", "left", "right"]}><ScrollView contentContainerStyle={styles.content}>
    ...
    <View style={[styles.publishCard, { backgroundColor: colors.surface, borderColor: song.isPublished ? `${colors.success}55` : colors.border }]}>
      ...
      <View style={styles.publishActions}>
        ...
        <Pressable onPress={async () => { try { await updateSong.mutateAsync({ id: song.id, isPublished: !song.isPublished, publishedVideoId: !song.isPublished ? song.publishedVideoId ?? null : null }); await utils.studio.snapshot.invalidate(); } catch (e) { Alert.alert("Změna se nezdařila", e instanceof Error ? e.message : "Zkus to znovu."); } }} disabled={updateSong.isPending} style={({ pressed }) => [styles.publishPrimary, { backgroundColor: song.isPublished ? colors.surface : colors.success, borderColor: song.isPublished ? colors.border : colors.success, opacity: updateSong.isPending || pressed ? 0.7 : 1 }]}><MaterialIcons name={song.isPublished ? "close" : "check"} size={16} color={song.isPublished ? colors.foreground : "#0A0A0F"} /><Text style={[styles.publishPrimaryText, { color: song.isPublished ? colors.foreground : "#0A0A0F" }]}>{song.isPublished ? "Zrušit publikaci" : "Označit jako publikované"}</Text></Pressable>
      </View>
    </View>
